...
def cleanenc(name,path,kv):
	...
	nname=""
	fil=[]
	f=open(os.path.join(os.path.dirname(os.path.realpath(__file__)),"cache/encfiles"),"rb")
	...
	if des!="":
		...
		if flag==0:
			...
			if nname!="":
				fil=[]
				file=open(os.path.join(os.path.dirname(os.path.realpath(__file__)),"cache/encfiles"),"rb")
				for i in file.readlines():
					fil.append(i.decode().strip("\n"))
				fil=set(fil)
				fil.remove(nname)
				file=open(os.path.join(os.path.dirname(os.path.realpath(__file__)),"cache/encfiles"),"wb")
				for i in fil:
